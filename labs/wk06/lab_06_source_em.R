...
# Purpose:  Store relevant lab 04 and 05 objects in this source file (for lab 06)
#
...
# Diff-in-Diff data set for LIHTC ----

# create a variable that identifies if a tract received LIHTC funding
treat2 <- as.numeric( d$num.lihtc > 0 )

# store the year 2000 data
d4 <- data.frame( y=y1, treat=treat2, post=0 )
# store the year 2010 data
d5 <- data.frame( y=y2, treat=treat2, post=1 )

# stack the two time periods together
d6 <- rbind( d4, d5 )


# Diff-in-Diff data set for LIHTC with control variables ----

# Create logs of control variables from lab 04
p.white <- log10( d$p.white.00 + 1  )
pov.rate <- log10( d$pov.rate.00 + 1 )
p.prof <- log10( d$p.prof.00 + 1 )

# Create additional variables for identifiers
cbsaname <- d$cbsaname
tractid <- d$tractid

# store new year 2000 data
d7 <- data.frame( y=y1, treat=treat2, post=0, p.white=p.white, 
                  pov.rate=pov.rate, p.prof=p.prof, cbsaname=cbsaname,
                  tractid=tractid )
# store new year 2010 data
d8 <- data.frame( y=y2, treat=treat2, post=1, p.white=p.white, 
                  pov.rate=pov.rate, p.prof=p.prof, cbsaname=cbsaname,
                  tractid=tractid )
