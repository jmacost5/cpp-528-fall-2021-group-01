...
df.instrument <- select(d, mhv.growth , p.white.00 , p.col.edu.00 , p.prof.00) %>%na.omit()
...
df.instrument$p.col.edu.00 <- log10( df.instrument$p.col.edu.00 + 1  )
...
reggres <- lm(mhv.growth ~ p.white.00 + p.col.edu.00 + p.prof.00 , data = df.instrument)
