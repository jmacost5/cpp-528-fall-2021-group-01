...
import::here("panel.cor",
             ...
             .from = here::here("analysis/utilities-amp.R"),
             .character_only = TRUE)
...
hist( d$p.hs, breaks=50, col="gray20", border="white",
      yaxt="n", xlab="", ylab="", main="Percent High School Degree+")

hist( log(d$p.hs+1), breaks=50, col="gray20", border="white",
...
d2 <- select( d, mhv.growth, p.vacant, p.hs, p.unemp)
...
reg.data$p.hs <- log10(reg.data$p.hs + 1)

#check for multicolinearity: p.vacant / p.hs

m1 <- lm( mhv.growth ~ p.vacant + cbsa, data = reg.data )
m2 <- lm( mhv.growth ~ p.hs + cbsa, data = reg.data )
m4 <- lm( mhv.growth ~ p.vacant + p.hs + cbsa, data = reg.data )
...
m3 <- lm( mhv.growth ~ p.unemp + cbsa, data = reg.data )
m5 <- lm( mhv.growth ~ p.vacant + p.unemp + cbsa, data = reg.data )
...
m6 <- lm( mhv.growth ~ p.unemp + p.hs + cbsa, data = reg.data )
...
m7 <- lm( mhv.growth ~ p.unemp + p.hs + p.vacant + cbsa, data = reg.data )
