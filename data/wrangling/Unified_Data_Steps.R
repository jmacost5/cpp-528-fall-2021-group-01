...
d <- dplyr::select( d, tractid, mhmval00, mhmval12, hinc00, hinc12, col00, col12,
      prof00, prof12, incpc00, incpc12, hs00, hs12, unemp00, unemp12, wds00, wds12,
      clf00, ag25up00, empclf00, empclf12, ag25up12, clf12,ag15up12,ag15up00,
      hu00 , vac00, own00, rent00, h30old00,family00 ,dpov00, npov00,pop00.x, 
      nhwht00,nhblk00, hisp00, asian00,cbsa, cbsaname , hu10,own10, rent10,
      dpov12,npov12,pop10,nhwht10,nhblk10,hisp10,asian10)


d <- 
  d %>%
  dplyr::mutate( # percent white in 2000
    ...
    pov.rate.percent = pov.rate.00 / 100, 
...
d3 <- data.frame( y=y1, treat=treat, post=0 )
# store the year 2010 data
d4 <- data.frame( y=y2, treat=treat, post=1 )

# stack the two time periods together
d5 <- rbind( d3, d4 )

# store the year 2000 data FOR LIHTC funding Program

# create a variable that identifies if a tract received LIHTC funding
treat2 <- as.numeric(d$num.lihtc > 0)

# store the year 2000 data
d6 <- data.frame( y=y1, treat2=treat2, post2=0 )
# store the year 2010 data
d7 <- data.frame( y=y2, treat2=treat2, post2=1 )
